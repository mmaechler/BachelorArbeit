...
$ \phi(- ; \mu, \Sigma) $ be the normal distribution with mean $ \mu $ and 
...
explain idea to use parameter optimizer instead, EM has pathological 
insufficiencies, like 'getting stuck' for many iterations. We hope we need less 
iterations, and as concequence less time. 'special' idea: using cholesky decomp.


\section{choice of notation}

% it 1
describe difference in notation between ceuleux \& govaert and our covariance 
...
Therefore we, from here on, when reffering to the decomposition as described by 
